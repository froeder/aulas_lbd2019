...
BEGIN 
     INSERT INTO comissao(id_cliente_funcionario, id_transacao, mes, valor) VALUES(NEW.id_cliente_funcionario, new.id_numero_contrato, EXTRACT(MONTH FROM now()), new.valor_comissao) ;
     RETURN NEW ;
END ;
...
-- Para atualizar historico_aluguel

CREATE OR REPLACE FUNCTION fn_atualiza_historico_aluguel() RETURNS TRIGGER AS $$
DECLARE parametro varchar:= '01' ;
BEGIN
    parametro := TG_ARGV[0] ;
    INSERT INTO historico_aluguel(id_cliente_usuario) VALUES (NEW.id_cliente_usuario, NEW.id_imovel, NEW.valor, parametro[0]) ;
    RETURN NEW ;
END;
$$ language 'plpgsql' ;


CREATE TRIGGER tr_atualizar_historico_aluguel 
AFTER INSERT OR UPDATE ON transacao 
FOR EACH ROW
EXECUTE PROCEDURE fn_atualiza_historico_aluguel('15')
